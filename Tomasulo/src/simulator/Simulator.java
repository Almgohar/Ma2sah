...
public class Simulator {
	...
	/*
	 * public void wTWallStore(String address, String newData) { int i = 0; Set
	 * theSet = null; Record theRecord = null; Record newRecord = null; for (i =
	 * 0; i < caches.size(); i++) { String[] brokenAddress =
	 * this.caches.get(i).breakAddress(address); theSet =
	 * this.caches.get(i).getSet(brokenAddress[1]); if (theSet != null) {
	 * theRecord = theSet.getRecord(brokenAddress[0]); if (theRecord != null)
	 * break; } } if (data == null) { data = simulator.memory.getValue(""
	 * +registerAddress); int offsetSize =
	 * simulator.caches.get(simulator.caches.size() - 1).getOffset(); int
	 * wordsToFetch = (int) Math.pow(2, offsetSize); String newAddress =
	 * decimalToBinary(registerAddress).substring(0,
	 * decimalToBinary(registerAddress).length() - offsetSize); for (int k = 0;
	 * k < offsetSize; k++) { newAddress += "0"; } String[] brokenAddress =
	 * simulator.caches.get(simulator.caches.size()-1).breakAddress(newAddress);
	 * int decimalAddress = simulator.binaryToDecimal(newAddress);
	 * 
	 * HashMap<String, String> newRecordM = new HashMap<String, String>();
	 * 
	 * int startOffset = 0; for (int k = 0; k < wordsToFetch;k++) { String
	 * newData = simulator.memory.getValue(""+ decimalAddress);
	 * if(decimalAddress == registerAddress) savedOffset =
	 * decimalToBinary(startOffset).substring(16-offsetSize, 16);
	 * newRecordM.put(decimalToBinary(startOffset).substring(16-offsetSize, 16),
	 * newData); System.out.println("some offset " +
	 * decimalToBinary(startOffset).substring(16-offsetSize, 16));
	 * startOffset++; decimalAddress++; }
	 * 
	 * Record newRecord = new Record(newRecordM); Set newSet = new Set(9);
	 * if(simulator
	 * .caches.get(simulator.caches.size()-1).cache.get(brokenAddress
	 * [1])==null){ System.out.println(brokenAddress[1] + "broken");
	 * simulator.caches
	 * .get(simulator.caches.size()-1).cache.put(brokenAddress[1],newSet);
	 * simulator
	 * .caches.get(simulator.caches.size()-1).cache.get(brokenAddress[1]
	 * ).records.put(brokenAddress[0], newRecord); } i = simulator.caches.size()
	 * - 1; } Register destination = simulator.getRegister(1, instArray);
	 * destination.setValue(decimalToBinary(Integer.parseInt(data))); Set theSet
	 * = null; Record theRecord = null; Record newRecord = null; for (int j = i
	 * - 1; j >= 0; j--) { int offSetLength =
	 * simulator.caches.get(j).getOffset(); String [] broken =
	 * simulator.caches.get(j +
	 * 1).breakAddress(decimalToBinary(registerAddress)); char offset =
	 * broken[2].charAt(0); HashMap<String, String> words = new HashMap<String,
	 * String>( (int) Math.pow(2, offSetLength));
	 * System.out.println("the offset is " + offset); int startIndex = 0; int
	 * endIndex = 0; if (offset == '1') { System.out.println(offSetLength +
	 * "legnth"); startIndex = offSetLength*2 / 2; endIndex =
	 * simulator.caches.get(j).getL()-1; } else { startIndex = 0; endIndex =
	 * (offSetLength*2 / 2) - 1; } theRecord =
	 * simulator.caches.get(j+1).cache.get(broken[1]).records.get(broken[0]);
	 * HashMap<String, String> oldWords = theRecord.words; int counter=0; for
	 * (String key : oldWords.keySet()) { // System.out.println(counter); //
	 * System.out.println("Start index " + startIndex) ; //
	 * System.out.println("End index " + endIndex) ; // if (counter >=
	 * startIndex && counter <= endIndex) { String newOffset =
	 * simulator.caches.get
	 * (j).breakAddress(decimalToBinary(registerAddress))[2]; registerAddress++;
	 * words.put(newOffset, oldWords.get(key)); // } counter++; } newRecord =
	 * new Record(words); String newIndex =
	 * simulator.caches.get(j).breakAddress(address)[1]; String newTag =
	 * simulator.caches.get(j).breakAddress(address)[0]; Set newSet =
	 * simulator.caches.get(j).cache.get(newIndex); if(newSet == null){ newSet =
	 * new Set(9); simulator.caches.get(j).cache.put(newIndex, newSet); } if
	 * (!newSet.canAddRecord()) { HashMap<String, Record> records =
	 * simulator.caches.get(j).cache .get(newIndex).records; int count = 0; for
	 * (String key : records.keySet()) { if (count == 0) {
	 * records.remove(records.get(key)); count++; break; } } }
	 * simulator.caches.get(j).cache.get(newIndex).insertRecord(newTag,
	 * newRecord); } for (int j = 0; j < caches.size(); j++) { String[]
	 * brokenAddress = caches.get(j).breakAddress(address); String elOffset =
	 * brokenAddress[2]; caches.get(j).getSet(brokenAddress[1]).records.put(
	...
	public void issue(Instruction instruction, ReservationStation resStation,
			int ROBIndex) {
		...
		if (instruction.getType().equals("FP")
			...
			if (regROB[rt] == -1) {
				...
				switch (rt) {
				case 0:
					regVk = registerFile.R0;
					break;
				case 1:
					regVk = registerFile.R1;
					break;
				case 2:
					regVk = registerFile.R2;
					break;
				case 3:
					regVk = registerFile.R3;
					break;
				case 4:
					regVk = registerFile.R4;
					break;
				case 5:
					regVk = registerFile.R5;
					break;
				case 6:
					regVk = registerFile.R6;
					break;
				case 7:
					regVk = registerFile.R7;
					break;
				default:
					regVk = null;
				}
	...
	public String execute(Instruction instruction) {
		...
		case "LW":
			if (instruction.getExecuteCycleCount() == 0) {
				String value = decimalToBinary(binaryToDecimal(station.getVj())
						+ binaryToDecimal(station.getAddress()));
		...
		case "SW":
			...
			return value;
		case "JMP":
			int calc = binaryToDecimal(registerFile.PC.getValue())
					+ binaryToDecimal(station.getVj()) + 1
					+ instruction.getImm();
		...
		case "JALR":
			...
			link.setValue(decimalToBinary(binaryToDecimal(registerFile.PC
					.getValue()) + 1));
			ROB.jFlush(ROBindex);
			return decimalToBinary(calc);
		case "RET":
		...
		default:
			return alu.arithmetic(instruction.getInst());
		}
	}
	 ...
	 * 
	 * IOException { int latency = 0;
	 * 
	 * 
	 * BufferedReader bf = new BufferedReader(new InputStreamReader(System.in));
	 * int numberOfCaches = Integer.parseInt(bf.readLine()); ArrayList<Tuple>
	 * tuples = new ArrayList<Tuple>(); int n = numberOfCaches; while (n > 0) {
	 * Tuple tuple = new Tuple(); tuple.setS(Integer.parseInt(bf.readLine()));
	 ...
	 * tuples.add(tuple); n--; }
	 * 
	 * int memoryCycles = Integer.parseInt(bf.readLine()); HashMap<String,
	 * String> iMemory = new HashMap<String, String>(); int numOfDataEntries =
	 * Integer.parseInt(bf.readLine()); while (numOfDataEntries > 0) { String
	 * memoryData = bf.readLine(); String[] splitData = memoryData.split(" ");
	 * iMemory.put(splitData[0], splitData[1]); numOfDataEntries--; }
	 * 
	 * String theAddress = bf.readLine(); String startAddress =
	 * decimalToBinary(Integer.parseInt(theAddress)); Simulator simulator = new
	 * Simulator(numberOfCaches, tuples, iMemory, memoryCycles);
	 * 
	 * simulator.registerFile.PC.setValue(decimalToBinary(Integer
	 * .parseInt(startAddress))); Arrays.fill(simulator.regROB,-1); String
	 * address = startAddress; int numberOfInstructions =
	 * Integer.parseInt(bf.readLine()); int numOfInstructions =
	 * numberOfInstructions; while (numberOfInstructions > 0) { String
	 * instruction = bf.readLine(); simulator.memory.iMemory.put(address,
	 * instruction); int decimalAddress = simulator.binaryToDecimal(address);
	 * decimalAddress++; address = decimalToBinary(decimalAddress);
	 * numberOfInstructions--;
	 * 
	 * } while (numOfInstructions > 0) { String instruction =
	 * simulator.memory.iMemory.get(startAddress); String[] instArray =
	 * instruction.split(" "); int registerAddress; switch (instArray[0]) { case
	 * "LW": simulator.setCache(simulator.caches.get(0)); registerAddress =
	 * Integer.parseInt(simulator.getRegister(2, instArray).getValue()) +
	 * Integer.parseInt(instArray[3]); String data =
	 * simulator.cache.load(decimalToBinary(registerAddress));
	 * 
	 * //latency += simulator.caches.get(0).getCycles(); int i = 1; while (data
	 * == null && i < simulator.caches.size()) {
	 * simulator.setCache(simulator.caches.get(i)); latency +=
	 * simulator.caches.get(i).getCycles(); String theRegisterAddress =
	 * simulator.decimalToBinary(registerAddress); data =
	 * simulator.cache.load(theRegisterAddress);
	 * simulator.cache.incrementEntries(); if (data != null){
	 * simulator.cache.incrementHits(); break; } i++; } String savedOffset="";
	 * //get data from memory and put it in the first cache level if (data ==
	 * null) { data = simulator.memory.getValue("" +registerAddress); int
	 * offsetSize = simulator.caches.get(simulator.caches.size() -
	 * 1).getOffset(); int wordsToFetch = (int) Math.pow(2, offsetSize); String
	 * newAddress = decimalToBinary(registerAddress).substring(0,
	 * decimalToBinary(registerAddress).length() - offsetSize); for (int k = 0;
	 * k < offsetSize; k++) { newAddress += "0"; } String[] brokenAddress =
	 * simulator.caches.get(simulator.caches.size()-1).breakAddress(newAddress);
	 * int decimalAddress = simulator.binaryToDecimal(newAddress);
	 ...
	 * records.remove(records.get(key)); count++; break; } } }
	 * simulator.caches.get(j).cache.get(newIndex).insertRecord(newTag,
	 * newRecord); } for(int l =0; l<simulator.caches.size(); l++){
	 * System.out.println("CACHE NUMBER " + (l+1));
	 * simulator.caches.get(l).print(); System.out.println(" "); }
	 * 
	 * break; case "SW": i = 0; simulator.setCache(simulator.caches.get(0));
	 * registerAddress = Integer.parseInt(simulator.getRegister(2,
	 * instArray).getValue()) + Integer.parseInt(instArray[3]); data =
	 * simulator.cache.load(decimalToBinary(registerAddress)); theSet = null;
	 * theRecord = null; newRecord = null; while (data == null && i <
	 * simulator.caches.size()) { simulator.setCache(simulator.caches.get(i));
	 * latency += simulator.caches.get(i).getCycles(); String theRegisterAddress
	 * = simulator.decimalToBinary(registerAddress); data =
	 * simulator.cache.load(theRegisterAddress);
	 * simulator.cache.incrementEntries(); if (data != null){
	 * simulator.cache.incrementHits(); break; } i++; } for (i = 0; i <
	 * simulator.caches.size(); i++) { String[] brokenAddress =
	 * simulator.caches.get(i).breakAddress(address); theSet =
	 * simulator.caches.get(i).getSet(brokenAddress[1]); if (theSet != null) {
	 * theRecord = theSet.getRecord(brokenAddress[0]); if (theRecord != null)
	 * break; } } if (data == null) { data = simulator.memory.getValue(""
	 * +registerAddress); int offsetSize =
	 * simulator.caches.get(simulator.caches.size() - 1).getOffset(); int
	 ...
	 * startOffset++; decimalAddress++; }
	 * 
	 * newRecord = new Record(newRecordM); Set newSet = new Set(9);
	 * if(simulator.
	 * caches.get(simulator.caches.size()-1).cache.get(brokenAddress[1])==null){
	 * System.out.println(brokenAddress[1] + "broken");
	 * simulator.caches.get(simulator
	 * .caches.size()-1).cache.put(brokenAddress[1],newSet);
	 * simulator.caches.get
	 * (simulator.caches.size()-1).cache.get(brokenAddress[1]
	 * ).records.put(brokenAddress[0], newRecord); } i = simulator.caches.size()
	 * - 1; } destination = simulator.getRegister(1, instArray);
	 * destination.setValue(decimalToBinary(Integer.parseInt(data))); theSet =
	 * null; theRecord = null; newRecord = null; for (int j = i - 1; j >= 0;
	 * j--) { int offSetLength = simulator.caches.get(j).getOffset(); String []
	 * broken = simulator.caches.get(j +
	 * 1).breakAddress(decimalToBinary(registerAddress)); char offset =
	 * broken[2].charAt(0); HashMap<String, String> words = new HashMap<String,
	 * String>( (int) Math.pow(2, offSetLength));
	 * System.out.println("the offset is " + offset); int startIndex = 0; int
	 ...
	 * records.remove(records.get(key)); count++; break; } } }
	 * simulator.caches.get(j).cache.get(newIndex).insertRecord(newTag,
	 * newRecord); } for (int j = 0; j < simulator.caches.size(); j++) {
	 ...
	 * //this.memory.iMemory.put(address, data); break; default:
	 * simulator.parser.parse(instruction); } int decimalAddress =
	 * Integer.parseInt(startAddress); decimalAddress++; startAddress = "" +
	 * decimalAddress; numOfInstructions--; //simulator.caches.get(0).print(); }
	 * simulator.getHitRatios(); System.out.println(latency); }
	 */
	...
	public static void main(String[] args) throws NumberFormatException,
		...
		for (int i = 0; i < num; i++) {
			...
			instructions[i] = new Instruction(instruction, status[0], false,
					getExecutionCycles(instruction, latencies));
		}

		ArrayList<ReservationStation> resStation = new ArrayList<ReservationStation>(
				11);
		String[] araf = { "LW", "SW", "BEQ", "JALR", "JMP", "RET", "ADD",
				"ADDI", "SUB", "MUL", "NAND" };
		for (int i = 0; i < 11; i++) {
